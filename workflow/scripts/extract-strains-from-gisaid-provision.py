...
def extract_strains_from_provision(
    ...
    for i, chunk in enumerate(chunks):
        ...
        provision = provision.append(select_strains_of_interest(chunk), ignore_index=True)
    provision = select_strains_of_interest(provision)
...
def select_strains_of_interest(df: pd.DataFrame):
    ...
    df = df[df["covv_lineage"].isin(snakemake.params.strains_of_interest)]
    # print(df["covv_lineage"])
...
def write_sequence(
    covv_lineage: str, covv_accession_id: str, covv_lineage_fasta: str, sequence: str, out_path: str
):
    ...
    if not os.path.isfile(genome_file):
        with open(genome_file, "w") as f:
            print(f">{covv_lineage}_{covv_accession_id}", file=f)
            print(f"{sequence}", file=f)
    else:
        with open(genome_file, "a") as f:
            print(f">{covv_lineage}_{covv_accession_id}", file=f)
            print(f"{sequence}", file=f)
